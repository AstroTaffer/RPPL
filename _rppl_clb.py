...
"FUCK THIS SHIT. TAKE A BREAK AND CHECK LATER"
# def make_master_dark(dark_frames, dark_frames_temp, out_filepath, min_frames_num=5, epsilon_temp=1):
#     """
#     :param dark_frames: numpy 1d array
#     :param dark_frames_temp: numpy 1d array
#     :param out_filepath: string
#     :param min_frames_num: integer
#     :param epsilon_temp: integer [C]
#     :return: bool
#     """
# 
#     mean_ccd_temp = np.round(np.mean(dark_frames_temp), 3)
#     dark_frames = dark_frames[np.where(abs(dark_frames_temp - mean_ccd_temp) <= epsilon_temp)]
# 
#     if len(dark_frames) < min_frames_num:
#         print(f"Less than {min_frames_num} dark frames with CCD-TEMP near {mean_ccd_temp} - can't make master dark")
#         return False
# 
#     buff_hdr = read_fits_file(dark_frames[0])[0]
#     dark_frames_data = np.zeros((len(dark_frames), buff_hdr["NAXIS1"], buff_hdr["NAXIS2"]), dtype=np.float32)
# 
#     for _ in range(len(dark_frames)):
#         dark_frames[_] = read_fits_file(dark_frames[_])[1]
# 
#     mdark_data = np.nanmean(sigma_clip(dark_frames, sigma=3, maxiters=5, masked=False, axis=0), axis=0)
# 
#     buff_hdr['BITPIX'] = -64
#     buff_hdr["DATE-OBS"] = ""
#     buff_hdr["ALPHA"] = ""
...
#     mdark_hdul.writeto(out_filepath, overwrite=True)
